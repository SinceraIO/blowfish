...
void bf_init_hex_key(Blowfish *this, char* hex_key);
void bf_init_bytes_key(Blowfish *this, byte* cipher_key, int key_length);
byte* bf_encrypt_ecb(Blowfish *this, byte* data, int length, int* new_length);
void bf_encrypt_block(Blowfish *this, byte* block, int offset);
byte* bf_decrypt_ecb(Blowfish *this, byte* data, int length, int* new_length);
void bf_decrypt_block(Blowfish *this, byte* block, int offset);
void bf_setup_key(Blowfish *this, byte* cipher_key, int length);
void bf_encipher(Blowfish *this);
void bf_decipher(Blowfish *this);
void bf_set_block(Blowfish *this, byte* block, int offset);
void bf_get_block(Blowfish *this, byte* block, int offset);
byte* bf_crypt_ecb(Blowfish *this, byte* data, int length, int* new_length, void (*crypt_block)(Blowfish*, byte*, int), int decrypt);
byte* bf_pad_data(Blowfish *this, byte* data, int length, int* paddedLength, int decrypt, int iv_space);
int bf_find_padding_end(Blowfish *this, byte* data, int length);
int bf_hex_to_dec(char hex);
unsigned int bf_round(Blowfish *this, unsigned int a, unsigned int b, unsigned int n);
...
void bf_init_hex_key(Blowfish *this, char* hex_key) {
    ...
    int key_length = strlen(hex_key) / 2;
    // Using Variable Length Array (VLA) to mimic stack allocation
    byte key[key_length];
    for (int i = 0; i < key_length; i++) {
        key[i] = bf_hex_to_dec(hex_key[i * 2]) * 16 + bf_hex_to_dec(hex_key[i * 2 + 1]);
    }
    bf_setup_key(this, key, key_length);
}
void bf_init_bytes_key(Blowfish *this, byte* cipher_key, int key_length) {
    bf_setup_key(this, cipher_key, key_length);
}
...
void bf_setup_key(Blowfish *this, byte* cipher_key, int length) {
    ...
    unsigned int d;
    for (int i = 0; i < 18; i++) {
        d = (((((unsigned int)key[j % length]) * 256 + key[(j + 1) % length]) * 256 + key[(j + 2) % length]) * 256 + key[(j + 3) % length]);
        p[i] ^= d;
        j = (j + 4) % length;
    }
...
byte* bf_encrypt_ecb(Blowfish *this, byte* data, int length, int* new_length) {
    return bf_crypt_ecb(this, data, length, new_length, &bf_encrypt_block, 0);
}

byte* bf_decrypt_ecb(Blowfish *this, byte* data, int length, int* new_length) {
    return bf_crypt_ecb(this, data, length, new_length, &bf_decrypt_block, 1);
}
...
byte* bf_pad_data(Blowfish *this, byte* data, int length, int* paddedLength, int decrypt, int iv_space) {
    ...
    if (decrypt) {
        ...
        *paddedLength = length;
        ...
        // if iv_space, leave a blank block at the front
        *paddedLength = 8 + ((length % 8 == 0) ? length : (length + 8 - (length % 8))) + (iv_space ? 8 : 0); //pad the data to a multiple of 8 plus one block
        if (iv_space)
            offset = 8;
    }
    // Allocate new array for padded data
    byte* pData = (byte*)malloc((*paddedLength) * sizeof(byte));
    for (int i = 0; i < length; i++)
...
int bf_find_padding_end(Blowfish *this, byte* data, int length) {
    int i = length;
    // find the first character from the back that isn't the same as the last character
    while (data[i - 1] == data[length - 1]) {
        i--;
    }
    return i; // return the length without the padding
}

byte* bf_crypt_ecb(Blowfish *this, byte* data, int length, int* new_length, void (*crypt_block)(Blowfish*, byte*, int), int decrypt) {
    byte* pData;
    pData = bf_pad_data(this, data, length, new_length, decrypt, 0); // this loads the IV from the front of the ciphertext

    // run the encryption
    for (int i = 0; i < *new_length; i += 8) {
        crypt_block(this, pData, i);
    }

    // if we are decrypting, we have to find where the data ends.
    if (decrypt) {
        *new_length = bf_find_padding_end(this, pData, *new_length);
    }
    return pData;
}

void bf_encrypt_block(Blowfish *this, byte* block, int offset) {
    bf_set_block(this, block, offset);
    bf_encipher(this);
    bf_get_block(this, block, offset);
}
...
void bf_set_block(Blowfish *this, byte* block, int offset) {
    ...
    this->xl_par = 0;
    for (int i = 0; i < 4; i++) {
        this->xl_par = (this->xl_par << 8) + block[offset + i];
        this->xr_par = (this->xr_par << 8) + block[4 + offset + i];
    }
}

void bf_get_block(Blowfish *this, byte* block, int offset) {
    unsigned int xl = this->xl_par;
    unsigned int xr = this->xr_par;
    for (int i = 3; i >= 0; i--) {
...
void bf_encipher(Blowfish *this) {
    ...
    for (int i = 0; i < ROUNDS; i += 2) {
        this->xr_par = bf_round(this, this->xr_par, this->xl_par, i + 1);
        this->xl_par = bf_round(this, this->xl_par, this->xr_par, i + 2);
    }
    this->xr_par ^= p[ROUNDS + 1];

    unsigned int swap = this->xl_par;
    this->xl_par = this->xr_par;
    this->xr_par = swap;
}

void bf_decipher(Blowfish *this) {
    this->xl_par ^= p[ROUNDS + 1];
    for (int i = ROUNDS; i > 0; i -= 2) {
        this->xr_par = bf_round(this, this->xr_par, this->xl_par, i);
        this->xl_par = bf_round(this, this->xl_par, this->xr_par, i - 1);
    }
    this->xr_par ^= p[0];
...
VALUE rb_decrypt(VALUE self, VALUE val) {
    if (RB_TYPE_P(val, T_STRING) != 1) {
      return Qnil;
    }
...
Init_blowfish(void)
{
  rb_cBlowfish = rb_define_class("Blowfish", rb_cObject);
  rb_define_singleton_method(rb_cBlowfish, "decrypt", rb_decrypt, 1);
}
